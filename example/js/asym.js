...
function generate(){
	...
	cipherJS.asym.simple.generateKeyset(bits, pass, function(keyset, carrier){
		x$('#keyset_generate').inner(keyset.keyset);
		x$('#publickeyset_generate').inner(keyset.publickeyset);
		hideOverlay();
...
function encrypt(){
	...
	pubkeys = pubkeys.split('<publickeyset>');
...
function sign(){
	...
	cipherJS.asym.simple.signMessage(cleartext, keyset, pass, function(signed, c){
...
function verify(){
	...
	var cleartext = message.between('<ct>', '</ct>');
	var keyset = getText('publickeyset_verify');
	if(message.length<1 || keyset.length<1){
		alert("It seems you forgot to fill some fields!");
		return;
	}
	cipherJS.asym.simple.verifyMessage(message, keyset, function(ver, c){
